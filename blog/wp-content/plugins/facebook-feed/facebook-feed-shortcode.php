...
function wff_shortcode_display ($atts) {
	...
	$facebookpageid = get_option('wff_page_id');
	
	$all_array =array(
	'page_id' => ( !empty( $facebookpageid ) ? $facebookpageid : '' ),
	...
	$wff_posts_json_url = 'https://graph.facebook.com/' . $page_id . '/posts?fields=id,from,message,message_tags,story,story_tags,link,picture,full_picture,attachments,source,updated_time,name,caption,description,type,status_type,object_id,created_time&access_token=' . $access_token . '&limit=' . $wff_post_limit;
